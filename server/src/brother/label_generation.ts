 ...
 * @returns 
 */
async function generateQRCodeImage(sampleID: string, size?: { width: number, height: number }): Promise<Buffer> {
    return await QRCode.toBuffer(sampleID, { ...(size ?? {}), type: 'png', margin: 0, errorCorrectionLevel: 'H' })
}
...
export async function generateLabelImageWithLayoutAndSample(layout: Label, sample: Sample): Promise<Sharp> {
    ...
    const qrCodeBuffer = await generateQRCodeImage(sample.id, { width: qrCodeSize, height: qrCodeSize });
