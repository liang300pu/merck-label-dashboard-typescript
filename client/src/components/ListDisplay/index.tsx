import { List, ListItem, ListItemButton, Paper, Divider, Button, Input } from "@mui/material";
...
export interface ListDisplayProps<T> {
    ...
    divideItems?: boolean;
    onListItemClick?: (item: ListDisplayItem, originalValue: T, index: number) => void;
    itemFormatter: (item: T, index: number) => ListDisplayItem;
    onListItemAdd?: () => void;
    onListItemAddAbort?: () => void;
    onListItemSubmit?: (value: string) => void;
    onListItemDelete?: (item: ListDisplayItem, originalValue: T, index: number) => void;
    editable?: boolean;
    deletable?: boolean;
...
function ListDisplay<T>({
    ...
    const onListItemDeleteClick = (item: ListDisplayItem, originalValue: T, index: number) => {
        onListItemDelete?.(item, originalValue, index);
    }

    return (
        <Paper className="list-display-container" style={styles}>
            {header}
            <List className="list-display-list">
                            ...
                            <ListItem>
                                {
                                    ...
                                    <Button 
                                        ...
                                        onClick={() => onListItemDeleteClick(formattedItem, item, index)}
                                    >
                                        <Delete />
                                    </Button> 
                                    : <></>
                                ...
                                <ListItemButton
                                    key={index}
                                    onClick={() => onListItemClick?.(formattedItem, item, index)}
                ...
                }
                {
                    ...
                    <Input
                        ...
                        onKeyDown={(event) => {
                            if (event.key == "Enter") {
                                onTemporaryListItemSubmit(event.currentTarget.value);
                            }
                        }}
                    />
                    :
