...
type FieldsType = Pick<TeamField, "name" | "display_name">;
...
const CreateSampleForm: React.FC = () => {
    ...
    useEffect(() => {
        if (team === '' || !fields.hasOwnProperty(team) || fields[team] === undefined) return;

        clearSampleData();

        setAllFields([...fields[team], ...requiredDateFields] ?? requiredDateFields);
    }, [team, fields]);
    ...
    const onTextFieldChange = (event: any, field: FieldsType) => {
        if (field.name in newSampleData) {
            setNewSampleData({
                ...newSampleData,
                [field.name]: DateTime.fromFormat(event.target.value, "yyyy-MM-dd")
            });
        } else {
            setNewSampleData({
                ...newSampleData,
    ...
    const onSubmitAndPrintButtonClick = () => {
        // TODO: Print?
        ...
        <>
            ...
            <Paper className="sample-form-paper">
                <form className="sample-form" autoComplete="off">
                    ...
                    <div
                        className="sample-form-button-container"
                    >
                        <Button
                            variant="contained"
